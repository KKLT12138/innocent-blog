...
function drawImg(width, height, line) {
  ...
  for (var i = 0; i < line; i++) {
    var color = '#' + ((Math.random() * 0x32505a + 0x288873) << 0).toString(16);
  ...
  drawText(numberArr[Math.floor(Math.random() * numberArr.length)][firstNumb], width * 0, 20);
  drawText(operationArr[operation], width * 0.2, 20);
  drawText(numberArr[Math.floor(Math.random() * numberArr.length)][secondNumb], width * 0.4, 20);
  drawText('=', width * 0.6, 20);
  drawText('?', width * 0.8, 20);
